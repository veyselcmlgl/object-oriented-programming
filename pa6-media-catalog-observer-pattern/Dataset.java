...
public class Dataset {
    ...
    public void notifyPlayerObserversForAdd()
    {
        for(Observer obs : observers)
        {
            ...
            {
                obs.addMessage(entries);  /* if the new player type entry is added all the player observers notify */
            }
        }
    }
    ...
    public void notifyViewerObserversForAdd()
    {
        for(Observer obs : observers)
        {
            ...
            {
                obs.addMessage(entries);  /* if the new viewer type entry is added all the viewer observers notify */
            }
        }
    }
    ...
    public void add(Entry newEntry)
    {
        ...
        {
            notifyPlayerObserversForAdd();  /* if entry is playable this functions calls */
        }
        else if(newEntry instanceof Non_playable)
        {
            notifyViewerObserversForAdd();  /* if entry is non_playable this functions calls */
        }
    }
    ...
    public void notifyPlayerObserversForDelete()
    {
        for(Observer obs : observers)
        {
            if(obs instanceof Player)
            {
                obs.removeMessage(entries);  /* if the player type entry is removed all the player observers notify */
            }
        }
    }
    ...
    public void notifyViewerObserversForDelete()
    {
        for(Observer obs : observers)
        {
            if(obs instanceof Viewer)
            {
                obs.removeMessage(entries);  /* if the viewer type entry is removed all the viewer observers notify */
            }
        }
    }
    ...
    public void remove(Playable deletedEntry)
    {
        ...
        notifyPlayerObserversForDelete();  /* if entry is playable this functions calls */
    }

    public void remove(Non_playable deletedEntry)
    {
        entries.remove((Entry) deletedEntry);  /* entry is removing of the arraylist */
        notifyViewerObserversForDelete();  /* if entry is non_playable this functions calls */
    }
